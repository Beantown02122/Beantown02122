# Terressa Jefferson

## IT Systems | Cybersecurity | Digital Forensics

---

## About Me

I am completing a Bachelor of Science in Information Technology with a concentration in Systems Administration and Cybersecurity, graduating May 2026. I design secure lab environments, deploy and manage Active Directory domains in Azure, conduct structured digital forensic investigations, and build Android applications using secure architecture principles. I focus on building, documenting, and securing real-world technical environments.

---

## Core Technical Focus

- Active Directory and Azure domain deployment  
- Network security lab design and vulnerability analysis  
- Digital forensics workflows and evidence validation  
- Secure Android application development using MVVM architecture  

---

## Featured Projects

### VirtualBox Penetration Testing Lab
Controlled lab environment simulating real-world vulnerability assessment using Kali Linux and Metasploitable.

### Azure Active Directory Domain Deployment
Enterprise-style Windows Server domain build with OU structure, Group Policy configuration, and identity management.

### Digital Forensics Investigation Lab
Forensic imaging, hash verification, file signature analysis, and tool validation aligned with industry standards.

### Senior Connect Android Application
Mobile application built with Kotlin, Room database, and structured architecture focused on usability and safety.

---

## Technical Stack

**Operating Systems**  
Windows Server | Linux | Kali Linux  

**Security Tools**  
Nmap | Wireshark | Metasploit | Autopsy | FTK Imager  

**Cloud & Identity**  
Microsoft Azure | Active Directory | Group Policy  

**Development**  
Kotlin | MVVM | Room Database | Firebase  

---
